...
export default function Portfolio() {
  const { isWalletConnected, connectWallet, walletAddress } = useAppStore();
  ...
  if (!isWalletConnected) {
    return (
      <div className="max-w-[1400px] mx-auto px-4 py-8">
        {/* Banner */}
        <div className="relative rounded-3xl overflow-hidden mb-8" style={{ background: 'linear-gradient(135deg, hsl(328 100% 30% / 0.5), hsl(270 60% 20% / 0.5), hsl(240 10% 8%))' }}>
          <div className="absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, hsl(328 100% 54% / 0.3), transparent 50%), radial-gradient(circle at 80% 30%, hsl(270 80% 60% / 0.2), transparent 40%)' }} />
          <div className="relative p-10 sm:p-16 text-center">
            ...
            <Button onClick={connectWallet} className="rounded-2xl px-8 h-12 text-base font-semibold">
              Connect Wallet
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-bold text-primary-foreground" style={{ background: 'linear-gradient(135deg, hsl(328 100% 54%), hsl(270 80% 60%))' }}>
          F
        </div>
        <div>
                ...
                <AreaChart data={portfolioChartData}>
                  <defs>
                    <linearGradient id="portfolioGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="hsl(328, 100%, 54%)" stopOpacity={0.3} />
                  ...
                  />
                  <Area type="monotone" dataKey="value" stroke="hsl(328, 100%, 54%)" strokeWidth={2} fill="url(#portfolioGrad)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
